...
values ('00020', 'visite_importee_compte')
on conflict (version) do nothing;


-- ============================================================
-- Vérification — les magasins dont un passage IMPORTÉ date désormais la dette.
-- Doit lister les magasins concernés avec une date de dernière visite.
-- ============================================================
select s.name as magasin,
       lv.last_visit_at::date as derniere_visite,
       d.jours_depuis_derniere_visite as jours
from stores s
join v_store_last_visit lv on lv.store_id = s.id
join v_store_dette d on d.store_id = s.id
where s.active
  and exists (select 1 from visits v
              where v.store_id = s.id and v.source is not null and v.active)
order by lv.last_visit_at desc nulls last
limit 20;

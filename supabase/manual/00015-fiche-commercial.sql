...
values ('00015', 'fiche_commercial')
on conflict (version) do nothing;


-- ============================================================
-- Vérification — une ligne par membre de l'équipe, prénom et nom séparés.
-- ============================================================
select nickname, first_name, last_name, full_name, email, phone, is_admin, active
from app_users
order by is_admin desc, nickname;

select case when exists (
         select 1 from pg_trigger where tgname = 'app_users_compose_full_name_trg'
       ) then 'OK — full_name se recompose tout seul'
         else 'MANQUANT' end as controle;

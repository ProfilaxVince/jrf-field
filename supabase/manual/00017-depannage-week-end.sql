...
values ('00017', 'depannage_week_end')
on conflict (version) do nothing;


-- ============================================================
-- Vérification — 3 lignes « OK » attendues.
-- ============================================================
select 'type depannage' as controle,
       case when exists (select 1 from pg_enum e join pg_type t on t.oid = e.enumtypid
                         where t.typname = 'visit_type' and e.enumlabel = 'depannage')
            then 'OK' else 'MANQUANT' end as resultat
union all
select 'colonne motif_depannage',
       case when exists (select 1 from information_schema.columns
                         where table_name = 'visits' and column_name = 'motif_depannage')
            then 'OK' else 'MANQUANT' end
union all
select 'motif obligatoire',
       case when exists (select 1 from pg_constraint
                         where conname = 'visits_depannage_motif_requis')
            then 'OK' else 'MANQUANT' end;

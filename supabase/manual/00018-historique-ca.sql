...
-- Vérification
-- ============================================================
select 'table store_revenues' as controle,
       case when exists (select 1 from information_schema.tables
                         where table_name = 'store_revenues') then 'OK' else 'MANQUANT' end as resultat
union all
select 'vue evolution',
       case when exists (select 1 from pg_views where viewname = 'v_store_revenue_evolution')
            then 'OK' else 'MANQUANT' end
union all
select 'log_audit reparee',
       case when (select prosrc from pg_proc where proname = 'log_audit') like '%v_apres%'
            then 'OK' else 'ANCIENNE VERSION' end;

-- Combien d'exercices repris ? (doit valoir le nombre de magasins ayant un CA)
select count(*) as exercices_repris,
       count(distinct store_id) as magasins_avec_ca,
       min(year) as plus_ancien, max(year) as plus_recent
from store_revenues;
